...
Dev modes: `./park --shot out.ppm TICKS [TOOL] [CAMX CAMY]` renders one frame
headlessly (`python3 tools/ppm2png.py out.ppm out.png` to view);
`./park --bench FRAMES WARMUP [MODE]` times frames; `./park --savetest TICKS`
saves a busy park to disk, loads it and compares.
...
| `win_frame.c` | A custom frame effect: Base's Linux window with a parallel blit that fills each quadtree square at once. |
